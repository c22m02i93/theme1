...
function news_portal_widgets_show_widget_field( $instance = '', $widget_field = '', $mt_widget_field_value = '' ) {
    ...
    switch ( $news_portal_widgets_field_type ) {
        ...
        <?php    
            ...
            ?>
                <p>
                    <input id="<?php echo esc_attr( $instance->get_field_id( $athm_option_name ) ); ?>" name="<?php echo esc_attr( $instance->get_field_name( $news_portal_widgets_name ).'['.$athm_option_name.']' ); ?>" type="checkbox" value="1" <?php checked('1', $mt_widget_field_value[$athm_option_name]); ?>/>
        ...
        case 'upload':
            ...
            <p><span class="field-label"><label for="<?php echo esc_attr( $instance->get_field_id( $news_portal_widgets_name ) ); ?>"><?php echo esc_html( $news_portal_widgets_title ); ?>:</label></span></p>
                ...
                <div class="actions np-clearfix">
                    <button type="button" class="button np-delete-button align-left"><?php _e( 'Remove', 'news-portal' ); ?></button>
                    <button type="button" class="button np-upload-button alignright"><?php _e( 'Select Image', 'news-portal' ); ?></button>
...
function news_portal_widgets_updated_field_value( $widget_field, $new_field_value ) {
    ...
    if ( $news_portal_widgets_field_type == 'number') {
        ...
        if ( !isset( $news_portal_widgets_allowed_tags ) ) {
            // If not, fallback to default tags
            $news_portal_widgets_allowed_tags = '<p><strong><em><a><iframe><span>';
        }
        return strip_tags( $new_field_value, $news_portal_widgets_allowed_tags );
