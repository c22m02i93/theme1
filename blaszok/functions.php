...
define('MPC_THEME_VERSION', '3.9.9.3');
...
if ( ! function_exists( 'mpcth_theme_setup' ) ) {
...
/* WooCommerce Additonal features support */
if ( ! function_exists( 'mpcth_additional_woo_features' ) ) {
	function mpcth_additional_woo_features() {
		global $mpcth_options;

		$woo_single_product_default = $mpcth_options[ 'mpcth_single_product_additional' ];
		$zoom                       = $mpcth_options[ 'mpcth_single_product_additional_zoom' ];
		$lightbox                   = $mpcth_options[ 'mpcth_single_product_additional_lightbox' ];

		if ( class_exists( 'WooCommerce' ) && ! empty( $woo_single_product_default ) ) {
...
add_filter( 'woocommerce_product_thumbnails_columns', 'mpcth_thumbnails_columns', 10 );
if ( ! function_exists( 'mpcth_thumbnails_columns' ) ) {
	function mpcth_thumbnails_columns(){
		return 5;
	}
}

/* WooCommerce jquery.cookie & js.cookie server issue workaround */
add_action('init', 'mpcth_woo_fix');
if ( ! function_exists( 'mpcth_woo_fix' ) ) {
	function mpcth_woo_fix() {
		if ( ! class_exists( 'WooCommerce' ) ) return;

		wp_register_script( 'jquery-cookie', MPC_THEME_URI . '/js/jquery.cokies' . ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min' ) . '.js', array( 'jquery' ), MPC_THEME_VERSION, true );
		wp_register_script( 'js-cookie', MPC_THEME_URI . '/js/js.cokie' . ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min' ) . '.js', array(), MPC_THEME_VERSION, true );
	}
}
...
if ( ! function_exists( 'mpcth_wp_title' ) ) {
...
/* WP Title - Separator
/* ---------------------------------------------------------------- */
add_filter('document_title_separator', 'mpcth_change_separator');
if ( ! function_exists( 'mpcth_change_separator' ) ) {
	function mpcth_change_separator() {
		return '|';
	}
}
...
if ( ! function_exists( 'mpcth_trim_excerpt' ) ) {
...
if ( ! function_exists( 'mpcth_theme_update' ) ) {
...
if ( ! function_exists( 'mpcth_admin_styles' ) ) {
	function mpcth_admin_styles() {
		global $mpcth_options;

		$enable_wpbakery_license = ( isset( $mpcth_options[ 'mpcth_enable_wpbakery_license' ] ) && $mpcth_options[ 'mpcth_enable_wpbakery_license' ]  ) ? true : false;

		if ( $enable_wpbakery_license ) {
...
if ( ! function_exists( 'mpcth_enqueue_scripts' ) ) {
	function mpcth_enqueue_scripts() {
		global $mpcth_options;
		global $page_id;
		$protocol = is_ssl() ? 'https' : 'http';

		$font_subset = ( $mpcth_options[ 'mpcth_font_subset' ] != 'default' ) ? ( '&amp;subset=' . $mpcth_options[ 'mpcth_font_subset' ] ) : '';
		...
		if ( isset( $mpcth_options[ 'mpcth_content_font' ] ) && is_array( $mpcth_options[ 'mpcth_content_font' ] ) ) {
			if ( $mpcth_options[ 'mpcth_content_font' ][ 'type' ] == 'google' ) {
				...
				wp_enqueue_style( 'mpc-content-font', apply_filters( 'blaszok/font', "$protocol://fonts.googleapis.com/css?family={$content_family}{$content_style}{$font_subset}" ) );
			}
		}
		if ( isset( $mpcth_options[ 'mpcth_heading_font' ] ) && is_array( $mpcth_options[ 'mpcth_heading_font' ] ) ) {
			if ( $mpcth_options[ 'mpcth_heading_font' ][ 'type' ] == 'google' ) {
				...
				wp_enqueue_style( 'mpc-heading-font', apply_filters( 'blaszok/font', "$protocol://fonts.googleapis.com/css?family={$heading_family}{$heading_style}{$font_subset}" ) );
			} else {
		...
		if ( isset( $mpcth_options[ 'mpcth_menu_font' ] ) && is_array( $mpcth_options[ 'mpcth_menu_font' ] ) ) {
			if ( $mpcth_options[ 'mpcth_menu_font' ][ 'type' ] == 'google' ) {
				...
				wp_enqueue_style( 'mpc-menu-font', apply_filters( 'blaszok/font', "$protocol://fonts.googleapis.com/css?family={$menu_family}{$menu_style}{$font_subset}" ) );
			}
		}

		if ( ! is_rtl() ) {
			wp_enqueue_style( 'mpc-styles', get_template_directory_uri() . '/style.css', array(), MPC_THEME_VERSION );

			if ( class_exists( 'WooCommerce' ) ) {
				wp_enqueue_style( 'mpc-woo-styles', get_template_directory_uri() . '/style-woo.css', array(), MPC_THEME_VERSION );
			}

			if ( class_exists( 'bbPress' ) ) {
				wp_enqueue_style( 'mpc-forum-styles', get_template_directory_uri() . '/style-forum.css', array(), MPC_THEME_VERSION );
			}

		} else {
			wp_enqueue_style( 'mpc-styles', get_template_directory_uri() . '/style-rtl.css', array(), MPC_THEME_VERSION );

			if ( class_exists( 'WooCommerce' ) ) {
				wp_enqueue_style( 'mpc-woo-styles', get_template_directory_uri() . '/style-woo-rtl.css', array(), MPC_THEME_VERSION );
			}

			if ( class_exists( 'bbPress' ) ) {
				wp_enqueue_style( 'mpc-forum-styles', get_template_directory_uri() . '/style-forum-rtl.css', array(), MPC_THEME_VERSION );
			}
		}

		if ( $mpcth_options[ 'mpcth_theme_skin' ] != 'default' ) {
			wp_enqueue_style( 'mpc-skins-styles', get_template_directory_uri() . '/css/' . $mpcth_options[ 'mpcth_theme_skin' ] . '.css', array(), MPC_THEME_VERSION );
		}

		$custom_styles_version = get_option( 'mpcth_unique_version_string' ) ?: MPC_THEME_VERSION;

		wp_enqueue_style( 'mpc-styles-custom', get_stylesheet_directory_uri() . '/style_custom.css', array(), $custom_styles_version );
...
#mpcth_footer_extended_section
	{ background-color: ' . $footer_ex_bg_color . '; ';
		...
		// wp_enqueue_style( 'font-awesome', MPC_THEME_URI . '/fonts/font-awesome.css', array(), MPC_THEME_VERSION );
		wp_enqueue_style( 'mpc-theme-plugins-css', MPC_THEME_URI . '/css/plugins.min.css', array(), MPC_THEME_VERSION );

		wp_enqueue_script( 'mpc-theme-plugins-js', MPC_THEME_URI . '/js/plugins.min.js', array( 'jquery' ), MPC_THEME_VERSION, true );
		wp_enqueue_script( 'mpc-theme-main-js', MPC_THEME_URI . '/js/main.min.js', array(
			'jquery',
			'mpc-theme-plugins-js'
		), MPC_THEME_VERSION, true );
		...
		$countdown_labels = array(
			...
			'label_minutes' => __( 'Minutes', 'mpcth' ),
...
/* ---------------------------------------------------------------- */
/* Custom styles unique version query string
/* ---------------------------------------------------------------- */
if ( ! function_exists( 'mpcth_update_unique_version_string' ) ) {
	function mpcth_update_unique_version_string( $ver_string ) {
		if ( get_option( 'mpcth_unique_version_string' ) !== false ) {
			update_option( 'mpcth_unique_version_string', $ver_string );
		} else {
			add_option( 'mpcth_unique_version_string', $ver_string );
		}
	}
}

if ( ! function_exists( 'mpcth_create_unique_version_string' ) ) {
	function mpcth_create_unique_version_string() {
		return date( 'YmdHis' );
	}
}
...
/* Sidebars Init
/* ---------------------------------------------------------------- */
add_action( 'widgets_init', 'mpcth_sidebars_init' );
if ( ! function_exists( 'mpcth_sidebars_init' ) ) {
	function mpcth_sidebars_init() {
...
/* ---------------------------------------------------------------- */
if ( ! function_exists( 'mpcth_get_sidebar_position' ) ) {
	function mpcth_get_sidebar_position() {
		...
		if ( is_search() ) {
			...
			if ( function_exists( 'is_woocommerce' ) && is_product_taxonomy() ) {
				...
				if ( $term_sidebar_position && $term_sidebar_position !== 'default' ) {
			...
			if ( is_category() || is_tag() ) {
				...
				$term_sidebar_position = get_field( 'mpc_sidebar_position', $queried_object );
...
/* Add social list
/* ---------------------------------------------------------------- */
if ( ! function_exists( 'mpcth_display_social_list' ) ) {
...
/* ---------------------------------------------------------------- */
if ( ! function_exists( 'mpcth_single_comment_template' ) ) {
				...
				<div class="mpcth-comment-header">
					...
					<a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ); ?>" class="mpcth-comment-permalink">
						<time class="mpcth-comment-date" datetime="<?php comment_time( 'c' ); ?>">
							<span>- </span><?php comment_date(); ?>
							<span> <?php _e( 'at', 'mpcth' ); ?> </span><?php comment_time(); ?>
						</time>
					</a>
...
/* Display pagination
/* ---------------------------------------------------------------- */
if ( ! function_exists( 'mpcth_display_pagination' ) ) {
...
/* Display load more
/* ---------------------------------------------------------------- */
if ( ! function_exists( 'mpcth_display_load_more' ) ) {
	function mpcth_display_load_more( $query ) {
		if ( $query->max_num_pages <= 1 ) {
			return;
		}
		?>
		<a href="#" id="mpcth_load_more" class="mpcth-color-main-background-hover"><?php _e( 'Load more', 'mpcth' ); ?>
			<span class="mpcth-load-more-icon"></span></a>
		<?php
		echo paginate_links( array(
...
/* Add lightbox
/* ---------------------------------------------------------------- */
if ( ! function_exists( 'mpcth_add_lightbox' ) ) {
...
/* Add secondary menu
/* ---------------------------------------------------------------- */
if ( ! function_exists( 'mpcth_display_secondary_menu' ) ) {
	function mpcth_display_secondary_menu() {
		...
		if ( class_exists( 'SitePress' ) ) {
			$language_switcher = ( ! empty( $mpcth_options[ 'mpcth_header_secondary_wpml_language' ] ) ) ? true : false;
			$currency_switcher = ( ! empty( $mpcth_options[ 'mpcth_header_secondary_wpml_currency' ] ) && defined( 'WCML_VERSION' ) ) ? true : false;

			if ( defined( 'ICL_SITEPRESS_VERSION' ) ) {

				if ( version_compare( ICL_SITEPRESS_VERSION, '3.7', '<' ) ) {
					...
					$mpc_theme = wp_get_theme();
					$dir_name  = sanitize_title( $mpc_theme->get( 'Name' ) );

					if ( $language_switcher ) {
						$template = $dir_name . '-flags';

 						if ( empty( $mpcth_options[ 'mpcth_header_secondary_wpml_language_default' ] ) ) {
							$template = $dir_name . '-default';
						}

						$native = ( empty( $mpcth_options[ 'mpcth_header_secondary_wpml_language_native' ] ) ) ? 0 : 1;
						$flags  = ( empty( $mpcth_options[ 'mpcth_header_secondary_wpml_language_flags' ] ) ) ? 0 : 1;
					...
					if ( $currency_switcher ) {
						$switcher_style = $dir_name . '-dropdown';

						if ( empty( $mpcth_options[ 'mpcth_header_secondary_wpml_currency_default' ] ) ) {
							$switcher_style = $dir_name . '-default';
						}

						$format = '%name% (%symbol%)';
						if ( ! empty( $mpcth_options[ 'mpcth_header_secondary_wpml_currency_code' ] ) ) {
							$format = '(%symbol%) %code%';
						}
		...
		if ( ! empty( $yith_wcwl ) && empty( $mpcth_options[ 'mpcth_header_secondary_wishlist_link' ] ) ) {
		...
		if ( class_exists( 'WooCommerce' ) && empty( $mpcth_options[ 'mpcth_header_secondary_account_link' ] ) ) {
			if ( ! is_user_logged_in() && get_option( 'woocommerce_myaccount_page_id' ) !== false ) {
				echo '<a href="' . get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) . '" class="mpcth-wp-login">' . __( 'Login', 'woocommerce' ) . '</a>';
			} else {
				echo '<a href="' . get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) . '" class="mpcth-wp-login">' . __( 'My account', 'woocommerce' ) . '</a>';
			}
		}
...
add_filter('woocommerce_product_description_tab_title', 'mpcth_product_description_tab_title');
if ( ! function_exists( 'mpcth_product_description_tab_title' ) ) {
	function mpcth_product_description_tab_title( $default ) {
		global $mpcth_options;
...
add_filter('woocommerce_product_additional_information_tab_title', 'mpcth_product_additional_information_tab_title');
if ( ! function_exists( 'mpcth_product_additional_information_tab_title' ) ) {
	function mpcth_product_additional_information_tab_title( $default ) {
		global $mpcth_options;
...
/* ---------------------------------------------------------------- */
if ( ! function_exists( 'mpcth_display_newsletter' ) ) {
...
/* ---------------------------------------------------------------- */
if ( ! function_exists( 'mpcth_display_secondary_header' ) ) {
...
/* Add post meta
/* ---------------------------------------------------------------- */
if ( ! function_exists( 'mpcth_add_meta' ) ) {
...
/* Excerpt
/* ---------------------------------------------------------------- */
if ( ! function_exists( 'mpcth_excerpt_ending' ) ) {
...
/* Theme updater
/* ---------------------------------------------------------------- */
require_once(MPC_THEME_PATH . '/php/updater.php');
if ( class_exists( 'MPC_Theme_Updater' ) ) {
	$MPC_Theme_Updater = new MPC_Theme_Updater( __DIR__ );
}

add_action( 'current_screen', 'mpcth_theme_update_notice' );
if ( ! function_exists( 'mpcth_theme_update_notice' ) ) {
	function mpcth_theme_update_notice() {
		global $mpcth_options;
		...
		if( ! isset( $mpcth_options[ 'mpcth_purchase_code' ] ) || strlen( $mpcth_options[ 'mpcth_purchase_code' ] ) !== 36 ) {
			add_action( 'admin_notices', 'mpcth_fillin_purchase_code' );

			return;
		}

		if ( $screen->base === 'update-core' || $screen->base === 'themes' ) {
			return;
		}

		$theme_name = wp_basename( __DIR__ );

		if ( isset( $themes->response[ $theme_name ] ) && isset( $themes->response[ $theme_name ][ 'new_version'] ) ) {
			$new_version = $themes->response[ $theme_name ][ 'new_version'];

			if ( version_compare( $new_version, MPC_THEME_VERSION , '>' ) ) {
				add_action( 'admin_notices', 'mpcth_update_available' );
			}
		}
	}
}

if ( ! function_exists( 'mpcth_update_available' ) ) {
	function mpcth_update_available() {
		echo '<div class="notice notice-warning"><p>' . __( '<strong>Blaszok</strong>: There is new theme update available. You can update from <a href="/wp-admin/update-core.php">Dashboard-&gt;Updates</a> or <a href="/wp-admin/themes.php">Appearance-&gt;Themes</a>.', 'mpc' ) . '</p></div>';
	}
}

if ( ! function_exists( 'mpcth_fillin_purchase_code' ) ) {
	function mpcth_fillin_purchase_code() {
		echo '<div class="notice notice-warning"><p>' . __( '<strong>Blaszok</strong>: Please add theme purchase code into <a href="/wp-admin/admin.php?page=mpcth-options-framework">Theme Options-&gt;Update</a>. This will allow easy theme updates directly from WordPress panel.', 'mpc' ) . '</p></div>';
	}
}
...
if ( ! function_exists( 'mpcth_theme_activation' ) ) {
...
if ( ! function_exists( 'mpcth_flush_rewrite_rules' ) ) {
...
// New Product Badge
add_action( 'woocommerce_before_shop_loop_item_title', 'mpcth_new_badge' );
add_action( 'woocommerce_product_thumbnails', 'mpcth_new_badge' );

if( ! function_exists( 'mpcth_new_badge' ) ) {
	function mpcth_new_badge() {
		...
		if ( ! isset( $mpcth_options['mpcth_enable_new_badge'] ) || ! $mpcth_options['mpcth_enable_new_badge'] ) {
			return;
		}

		if ( ! isset( $mpcth_options['mpcth_new_badge_time'] ) ) {
			return;
		}

		$new_badge_time = ( floatval( $mpcth_options['mpcth_new_badge_time'] ) ?: 14 ) * DAY_IN_SECONDS;
		$current_time   = time();

		$product_time     = $product->get_date_created();
		$product_time_sec = $product_time->date( 'U' );

		if ( $product_time_sec > $current_time - $new_badge_time ) {
			echo '<div class="mpcth-sale-wrap new"><span class="onsale new">' . __( 'New', 'mpcth' ) . '</span></div>';
		}
	};
}
...
function mpc_custom_breadrumb_home_url() {
	...
	if ( is_product( $current_page_id ) && $shop_page_id != $frontpage_id ) {
...
function mpcth_custom_product_tabs( $tabs ) {
	...
	if ( function_exists( 'icl_object_id' ) ) {
		$translated_post_id = apply_filters( 'wpml_object_id', $post_id, 'product' );
	} else {
		$translated_post_id = $post->ID;
	}

	// check with: mpc_custom_tab
	$custom_tabs = get_field( 'mpc_custom_tab', $translated_post_id );
...
add_filter('woocommerce_add_to_cart_fragments', 'mpcth_wc_ajaxify_mini_cart_icon');
function mpcth_wc_ajaxify_mini_cart_icon($fragments) {
	...
	<span class="mpcth-mini-cart-icon-info">
		<?php if (sizeof( WC()->cart->get_cart()) > 0) { ?>
			<span class="mpcth-mini-cart-subtotal"><?php echo $subtotal; ?> </span><?php echo WC()->cart->get_cart_subtotal(); ?> (<?php echo WC()->cart->get_cart_contents_count(); ?>)
		<?php } ?>
	</span>
...
add_filter('woocommerce_add_to_cart_fragments', 'mpcth_wc_ajaxify_mini_cart');
		...
		<?php if ( sizeof( WC()->cart->get_cart() ) > 0 ) : ?>
			<ul class="mpcth-mini-cart-products">
				<?php foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) :
					...
					if( !empty( $product_id ) )
						$variations_data = wc_get_formatted_cart_item_data( $cart_item, true );
					...
					<li class="mpcth-mini-cart-product">
						<span class="mpcth-mini-cart-thumbnail">
							...
							<?php echo apply_filters( 'woocommerce_cart_item_remove_link', '<a href="' . esc_url( wc_get_cart_remove_url( $cart_item_key ) ) . '" class="mpcth-mini-cart-remove mpcth-color-main-color" title="' . __( 'Remove this item', 'woocommerce' ) . '">&times;</a>', $cart_item_key ); ?>
						</span>
...
if ( ! function_exists( 'mpcth_vc_update' ) ) {
	function mpcth_vc_update() {
		global $mpcth_options;

		$enable_wpbakery_license = ( isset( $mpcth_options[ 'mpcth_enable_wpbakery_license' ] ) && $mpcth_options[ 'mpcth_enable_wpbakery_license' ]  ) ? true : false;

		if ( ! $enable_wpbakery_license && function_exists( 'vc_manager' ) ) {
...
/* Blog
/* ---------------------------------------------------------------- */
function mpcth_print_blog_archives_custom_header() {
	$queried_object      = get_queried_object();
	$header_content_type = get_field( 'mpc_header_content_type', $queried_object );
	$custom_header       = get_field( 'mpc_custom_header', $queried_object );
	$image_header        = get_field( 'mpc_image_header', $queried_object );
...
/* Page excerpt field for "Slider Post" testimonials editing */
add_action( 'init', 'mpcth_add_excerpts_to_pages' );

if ( ! function_exists( 'mpcth_add_excerpts_to_pages' ) ) {
	function mpcth_add_excerpts_to_pages() {
    	add_post_type_support( 'page', 'excerpt' );
	}
}

add_action( 'current_screen', 'mpcth_subscribe2_conflict_fix' );
if ( ! function_exists( 'mpcth_subscribe2_conflict_fix' ) ) {
	function mpcth_subscribe2_conflict_fix() {
		global $pagenow;

		if ( ! class_exists( 'S2_Block_Editor' ) ) {
			return;
		}
		...
		if ( ! method_exists( $screen, 'is_block_editor' ) || ! $screen->is_block_editor() || ! isset( $_GET[ 'vcv-gutenberg-editor' ] ) ) {
			//  It is crashing Yoast SEO for non-gutenberg screen
			add_action( 'wp_print_scripts', 'mpcth_dequeue_subscribe2_script', 100 );
		}
	}
}

if ( ! function_exists( 'mpcth_dequeue_subscribe2_script' ) ) {
	function mpcth_dequeue_subscribe2_script(){
		wp_dequeue_script( 'subscribe2-sidebar' );
	}
}

add_action( 'after_setup_theme', 'mpcth_wishlist_default_position' );
if ( ! function_exists( 'mpcth_wishlist_default_position' ) ) {
	function mpcth_wishlist_default_position(){
