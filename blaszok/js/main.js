...
/* ---------------------------------------------------------------- */
	...
	$('#main_slider').flexslider({
		...
		sync: '#main_thumbs',
		start: function(){
			$('#main_slider').addClass( 'slider-loaded' );
		}
	});
	...
	if ( $( '#main_slider' ).length && $( '#main_thumbs').length ) {
		$( '.variations select').on( 'change', function(){
			$( '#main_slider' ).data('flexslider').flexAnimate( 0 );
			$( '#main_thumbs').data('flexslider').flexAnimate( 0 );
		});
	}
	...
	$carousels.each(function() {
		...
		if ( $carousel.is('.mpc-vc-images-slider') ) {
			if ( $carousel.attr('data-speed') != '' ) {
				...
				carousel_defaults.auto.timeoutDuration = parseInt( $carousel.attr('data-speed') );
			}

			if ( $carousel.attr('data-single') == '1' ) {
				carousel_defaults.auto.items = 1;
				carousel_defaults.scroll.items = 1;
			}

			if ( $carousel.attr('data-loop') != '1' ) {
		...
		if( $carousel.is( '.mpc-vc-products-slider' ) ) {
			...
			if ( $carousel.attr('data-single') == '1' && $carousel.attr('data-single') != undefined ) {
				carousel_defaults.auto.items = 1;
				carousel_defaults.scroll.items = 1;
			}

			if ( $carousel.attr('data-speed') != '' && $carousel.attr('data-speed') != undefined ) {
				carousel_defaults.auto.play = true;
				carousel_defaults.auto.timeoutDuration = parseInt( $carousel.attr('data-speed') );
			}
        ...
        function mpcth_badge_size() {
            $('.mpcth-badge-circle').each( function() {
                var $this = $( this ),
					...
					square_size = Math.max( el_height, el_width);
...
/* ---------------------------------------------------------------- */
	...
	/* Sold Out Wrap on Single Product */
	var $single_product_gallery = $('.single-product .woocommerce-product-gallery'),
		$sold_out_wrap          = $single_product_gallery.find('.mpcth-sale-wrap.sold'),
		$new_wrap               = $single_product_gallery.find('.mpcth-sale-wrap.new');

	if ( $sold_out_wrap.length ){
		$sold_out_wrap.detach().appendTo( $single_product_gallery );
	}

	if ( $new_wrap.length ){
		$new_wrap.detach().appendTo( $single_product_gallery );
	}
...
/* ---------------------------------------------------------------- */
	...
	// WPML
	$( '.mpcth-wpml-vanila-language-switcher' ).customSelect({customClass: 'mpcthSelect'});

	$( '.mpcth-wpml-vanila-language-switcher' ).change( function( event ) {
		event.preventDefault();

		window.location = $( this ).find( 'option:selected' ).val();
	} );

	$( '.mpcth-wpml-vanila-currency-switcher' ).customSelect( { customClass: 'mpcthSelect' } );

	$( '.mpcth-wpml-vanila-currency-switcher' ).change( function( event ) {
		event.preventDefault();

		$( this ).parent().append( '<span class="mpcth-load-more-icon active"></span>' );

		wcml_load_currency( $( this ).attr( 'value' ) );
	} );

	$( '.mpcth-wpml-drop-currency-switcher .mpcth-wpml-sub-dropdown a' ).on( 'click', function( event ) {
		event.preventDefault();

		$( this ).closest( '.mpcth-wpml-item-current' ).append( '<span class="mpcth-load-more-icon active"></span>' );
		$( this ).closest( '.mpcth-wpml-sub-dropdown' ).hide();

		wcml_load_currency( $( this ).attr( 'rel' ) );
	} );

	$( '.mpcth-wpml-item-current > a' ).on( 'click', function( event ) {
		event.preventDefault();
	} );
...
/* ---------------------------------------------------------------- */
	$( window ).on( 'load', function() {
		...
		if ( typeof essapi_2 === 'undefined' || typeof essapi_4 === 'undefined' ) {
			mpcth_fixed_footer();
		} else {
			// Run Sticky Footer after Essential Grid is loaded
			var gridLoaded = setInterval(function() {

				if( essapi_2.is( ':visible' ) || essapi_4.is( ':visible' ) ) { // essapi_4 = current grid's API Name
...
(function($) {
	var $mobileMenu = $( '.mpcth-accordion-menu #mpcth_mobile_menu' );

	if ( $mobileMenu.length < 1 ) {
		return;
	}

	var $subMenus = $mobileMenu.find( '.menu-item-has-children' );

	$subMenus.each( function() {
		var $subMenuItems = $( this ).children( '.sub-menu' ),
			$button       = $( this ).find( '> a' );

		$subMenuItems.slideUp();
		$button.append( '<i class="mpcth-menu-button fa fa-plus"></i>' );
	});

	$( '#mpcth_mobile_menu .mpcth-menu-button' ).on( 'click', function( event ){
		event.preventDefault();

		var $subMenuButton = $( this );
			$directSubMenu = $subMenuButton.parent().next( '.sub-menu' );

		if ( $subMenuButton.hasClass( 'fa-plus' ) ) {
			$subMenuButton.removeClass( 'fa-plus' );
			$subMenuButton.addClass( 'fa-minus' );
		} else {
			$subMenuButton.removeClass( 'fa-minus' );
			$subMenuButton.addClass( 'fa-plus' );
		}

		$directSubMenu.slideToggle( 300 );
	} );
