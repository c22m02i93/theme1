...
if ( is_admin() ) {
	...
	//$of_page= 'elements_page_mpcth-options-framework';
	if (isset($_REQUEST['page']) && !('yith_wcact_panel_product_auction' == $_REQUEST['page'])) {
		add_action( "admin_print_styles", 'mpcth_optionsframework_mlu_css', 0 );
		add_action( "admin_print_scripts", 'mpcth_optionsframework_mlu_js', 0 );
	}
}
 ...
 */
if ( ! function_exists( 'mpcth_optionsframework_mlu_init' ) ) {
	function mpcth_optionsframework_mlu_init () {
		register_post_type( 'mpcth_of', array(
			'labels' => array(
				'name' => __('Theme Options Media', 'mpcth'),
			),
			'public' => true,
			'show_ui' => false,
 ...
 */
if ( ! function_exists( 'mpcth_optionsframework_mlu_css' ) ) {
	function mpcth_optionsframework_mlu_css () {
		$_html = '';
 ...
 */
if ( ! function_exists( 'mpcth_optionsframework_mlu_js' ) ) {
	function mpcth_optionsframework_mlu_js () {
		// Registers custom scripts for the Media Library AJAX uploader.
		wp_register_script( 'mpcth-of-medialibrary-uploader', MPC_THEME_URI . '/panel/js/of-medialibrary-uploader.js', array( 'jquery', 'thickbox' ) );
		wp_enqueue_script( 'mpcth-of-medialibrary-uploader' );
 ...
 */
if ( ! function_exists( 'mpcth_optionsframework_medialibrary_uploader' ) ) {
	function mpcth_optionsframework_medialibrary_uploader( $_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0, $_name = '', $_simple_name = false  ) {
		...
		$int = mpcth_optionsframework_mlu_get_silentpost( $id );
		...
		if ($_name != '') {
			if($_simple_name)
				...
				$name = $mpcth_option_name.'['.$id.']['.$_name.']';
		}
		else {
			$name = $mpcth_option_name.'['.$id.']';
		}

		if ( $value ) { $class = ' has-file'; }
		$output .= '<input id="' . $id . '" class="of-input upload' . $class . '" type="text" name="'.$name.'" value="' . $value . '" />' . "\n";
		$output .= '<input id="upload_' . $id . '" class="upload_button button mpcth-of-gray-button" type="button" value="' . __( 'Upload', 'optionsframeworktheme' ) . '" rel="' . $int . '" />' . "\n";
		...
		if ( $value != '' ) {
			$remove = '<a href="javascript:(void);" class="mlu_remove mpcth-mp-icon-cancel"></a>';
			$image = preg_match( '/(^.*\.jpg|jpeg|png|gif|ico|svg|svgz*)/i', $value );
			if ( $image ) {
				...
				$title = __( 'View File', 'mpcth_optionsframework' );
 ...
 */
if ( ! function_exists( 'mpcth_optionsframework_mlu_get_silentpost' ) ) {
	function mpcth_optionsframework_mlu_get_silentpost ( $_token ) {
		global $wpdb;
		$_id = 0;
		...
		if ( $_token ) {
			...
			$_args = array( 'post_type' => 'mpcth_of', 'post_name' => 'mpcth-of-' . $_token, 'post_status' => 'draft', 'comment_status' => 'closed', 'ping_status' => 'closed' );
 ...
 */
if ( ! function_exists( 'mpcth_optionsframework_mlu_insidepopup' ) ) {

	function mpcth_optionsframework_mlu_insidepopup () {

		if ( isset( $_REQUEST['is_mpcth_optionsframework'] ) && $_REQUEST['is_mpcth_optionsframework'] == 'yes' ) {

			add_action( 'admin_head', 'mpcth_optionsframework_mlu_js_popup' );
			add_filter( 'media_upload_tabs', 'mpcth_optionsframework_mlu_modify_tabs' );
		}
	}
}

if ( ! function_exists( 'mpcth_optionsframework_mlu_js_popup' ) ) {

	function mpcth_optionsframework_mlu_js_popup () {
 ...
 */
if ( ! function_exists( 'mpcth_optionsframework_mlu_modify_tabs' ) ) {

	function mpcth_optionsframework_mlu_modify_tabs ( $tabs ) {
		$tabs['gallery'] = str_replace( __( 'Gallery', 'mpcth' ), __( 'Previously Uploaded', 'mpcth' ), $tabs['gallery'] );
		return $tabs;
	}
}