...
function mpcth_optionsframework_load_scripts($hook) {
	...
	wp_localize_script('mpcth-options-custom', 'mpcthLocalize', array(
		...
		'googleAPIErrorMsg' => __('There is problem with access to Google Webfonts. Please try again later. If this message keeps appearing please contact our support at <a href="https://mpc.ticksy.com/">mpc.ticksy.com</a>.', 'mpcth'),
...
?>
	<div id="mpcth-of-wrap" class="">
		...
		<div id="mpcth_update_msg">
			...
			<p><?php _e('We try our best to make it issues free but some bugs might still be there waiting to appear. If you found any issues or need help, please feel free to send us a support ticket and we will write back to you as soon as possible :) Visit ', 'mpcth'); ?><a href="https://mpc.ticksy.com" target="_blank"><?php _e('our support', 'mpcth'); ?></a>.</p>
		</div>
		<?php } ?>
		...
		<div id="mpcth-of-metabox" class="">
			<div id="mpcth-of-header">
				...
				<a href="https://mpc.ticksy.com" class="mpcth-of-support" target="_blank"><?php _e('Support', 'mpcth'); ?></a>
			</div>
			...
			<div class="mpcth-of postbox">
				...
				<?php wp_nonce_field( 'mpcth_options_page_security' ); ?>
				...
				<div id="optionsframework-submit">
					...
					<div class="ajax-message" class="">
						<div class="ajax-label ajax-saved"><?php _e('Splendid! Options and styles saved.', 'mpcth'); ?></div>
						<div class="ajax-label ajax-options"><?php _e('Only options were saved. Try again.', 'mpcth'); ?></div>
						<div class="ajax-label ajax-styles"><?php _e('Styles has been regenerated.', 'mpcth'); ?></div>
...
function mpcth_optionsframework_validate( $input ) {
	 ...
	 */
		...
		foreach ($mpcth_options as $mpcth_option) {
			...
			if ( isset( $input[ $id ] ) && has_filter( 'mpcth_of_sanitize_' . $mpcth_option['type'] ) ) {
...
function mpcth_save_panel_options() {
	...
	if ( ! isset( $_POST[ 'data' ] ) || ! isset( $_POST[ '_wpnonce' ] ) || ! current_user_can( 'manage_options' ) ) {
		wp_send_json_error();
	}

	check_ajax_referer( 'mpcth_options_page_security' );
	...
	if ( $options === $clean ) {
		$result = mpcth_update_custom_styles( null, $clean, null );

		die((string)$result);
	}

	add_action( 'update_option_mpcth_options_theme_customizer', 'mpcth_update_custom_styles', 10, 3 );

	update_option('mpcth_options_theme_customizer', $clean );

	$result = 3;

	remove_action( 'update_option_mpcth_options_theme_customizer', 'mpcth_update_custom_styles', 10 );
...
function mpcth_export_settings() {
	if ( ! current_user_can( 'manage_options' ) ) {
		exit( __( 'You are not allowed to do this', 'mpcth' ) );
	}
...
function mpcth_update_custom_styles( $old_value = null, $value = null, $option = null ) {
	$mpcth_options = $value;
...
.mpcth-skin-dark .mpcth-post-content-wrap .mpcth-post-title a:hover
	...
	/* Header type menu/logo/search */
	if ( isset( $mpcth_options['mpcth_header_main_layout'] ) && $mpcth_options['mpcth_header_main_layout'] === 'm_l_s' ) {
		if ( isset( $mpcth_options['mpcth_logo_width'] ) && $mpcth_options['mpcth_logo_width'] !== '' ) {
			...
			#mpcth_page_wrap #mpcth_page_header_content.mpcth-header-order-m_l_s #mpcth_logo_wrap {
				min-width: {$mpcth_options['mpcth_logo_width']}px;
			}
			";
		}
		...
		if ( isset( $mpcth_options['mpcth_logo_mobile_width'] ) && $mpcth_options['mpcth_logo_mobile_width'] !== '' ) {
			...
			@media only screen and (max-width: 767px){
				#mpcth_page_wrap #mpcth_page_header_content.mpcth-header-order-m_l_s #mpcth_logo_wrap {
					min-width: {$mpcth_options['mpcth_logo_mobile_width']}px;
				}
			}
			";
		}
		...
		if ( isset( $mpcth_options['mpcth_logo_sticky_width'] ) && $mpcth_options['mpcth_logo_sticky_width'] !== '' ) {
			...
			#mpcth_page_wrap .mpcth-sticky-header #mpcth_page_header_content.mpcth-header-order-m_l_s #mpcth_logo_wrap {
				min-width: {$mpcth_options['mpcth_logo_sticky_width']}px;
			}
			";
		}
	}
...
#mpcth_page_wrap .vc_btn3 { background-color: $main_color; }
div.vc_progress_bar .vc_single_bar .vc_bar { background-color: $main_color; }
...
body #mpcth_page_header_content #mpcth_nav
	...
	if($saved !== false) {
		...
		$unique_version = mpcth_create_unique_version_string();

		mpcth_update_unique_version_string( $unique_version );

		return 2;
	} else {
		return 0;
		//TODO if possible AJAX output for not saved styles
	}
}
...
if ( !function_exists( 'mpcth_log_it' ) ) {
	function mpcth_log_it( $message ) {
		if ( WP_DEBUG === true ) {
...
if ( !function_exists( 'mpcth_add_svg_support' ) ) {
	function mpcth_add_svg_support( $mimes ) {
...
add_filter( 'upload_mimes', 'mpcth_add_svg_support' );