...
/* Update logo width
/* ---------------------------------------------------------------- */
	const $mpcOptions        = $( '#mpcth-of-wrap' ),
		  $logoWidthSections = $mpcOptions.find( '.mpcth_logo_width' ),
		  $widthInputs       = $logoWidthSections.find( '.of-input' );

	if ( $mpcOptions.length < 1 || $logoWidthSections.length < 1 || $widthInputs.length < 1 ) {
		return;
	}

	$widthInputs.attr( 'type', 'number' );
	$widthInputs.attr( 'max', '1920' );

	$widthInputs.on( 'keyup mouseup', function(){
		updateImageWidth( $( this ) );
	} );

	$widthInputs.each( function(){
		updateImageWidth( $( this ) );
	});

	$mpcOptions.on( 'mpcth.imageLoaded', function(){
		$widthInputs.each( function(){
			updateImageWidth( $( this ) );
		});
	} );

	function updateImageWidth( $element ) {
		var $section     = $element.closest( '.mpcth-of-section' ),
			$prevSection = $section.prev( '.mpcth-of-section' ),
			$nextSection = $section.next( '.mpcth-of-section' ),
			$logo        = $prevSection.find( 'img'),
			$retinaLogo  = $nextSection.find( 'img'),
			value        = Math.min( parseInt( $element.val() ), 1920 );

		if ( $logo.length ) {
			$logo.css( 'width', value + 'px' );
		}

		if ( $retinaLogo.length ) {
			$retinaLogo.css( 'width', value + 'px' );
		}
	}
});
