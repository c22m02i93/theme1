...
$css_classes = array(
	...
	'vc_row-fluid',
	'mpcth-vc-row-wrap',
	( $mpc_atts[ 'bg_image_fb' ] || $mpc_atts[ 'bg_image' ] || $parallax || $parallax_image || $video_bg_parallax ? ' mpcth-vc-row-wrap-image ' : ''),
...
if ( ! empty ( $parallax_image ) || ! empty( $parallax ) || ! empty( $mpc_atts[ 'bg_image_fb' ] ) || ! empty( $mpc_atts[ 'bg_image' ] ) ) {
	if ( $has_video_bg ) {
		$parallax_image_src = $parallax_image;
	} else if ( $mpc_atts[ 'bg_image_fb' ]
		|| $mpc_atts[ 'bg_image_repeat_fb' ]
		|| $mpc_atts[ 'bg_image' ]
		|| $mpc_atts[ 'bg_image_repeat' ] ) {

		if ( $mpc_atts[ 'bg_image_fb' ] ) {
			$image_src = wp_get_attachment_image_src( $mpc_atts[ 'bg_image_fb' ], 'full' );
		} elseif( $mpc_atts[ 'bg_image' ] ) {
			$image_src = wp_get_attachment_image_src( $mpc_atts[ 'bg_image' ], 'full' );
		}

		$parallax_image_src = $image_src;

		if ( ! empty( $parallax_image_src[0] ) ) {
			$parallax_image_src = $parallax_image_src[0];
		}

		if( ! empty( $mpc_atts[ 'bg_image_repeat_fb' ] ) ) {
			$wrapper_attributes[] = ' style="background-repeat:' . $mpc_atts[ 'bg_image_fb' ]. ';"';
		} else if( ! empty( $mpc_atts[ 'bg_image_repeat' ] ) ) {
			$wrapper_attributes[] = ' style="background-repeat:' . $mpc_atts[ 'bg_image' ]. ';"';
		}
	} else {
